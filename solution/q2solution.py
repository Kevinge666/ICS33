...
def pages(page_spec: str, unique: bool) -> [int]:  # result in ascending order
    ...
    for ps in page_spec.split(','):
        ...
        assert m is None, 'pages: Illegal specification: ' + ps
        p1, sep, p2, n = m.group(1, 2, 3, 4)
        if sep is None:
            answer.append(int(p1))
        else:
            p1, p2, n = int(p1), int(p2), (1 if n is None else int(n))
            if sep == '-':
...
if __name__ == '__main__':
    ...
    for text in open('bm1.txt'):
        ...
        m = re.match(p1a, text)
        print(' ', 'Matched' if m is not None else "Not matched")
    ...
    for text in open('bm1.txt'):
        ...
        print('Matching against:', text)
        m = re.match(p1b, text)
        print('  ', 'Matched with groups =' + str(m.groups()) if m is not None else 'Not matched')
